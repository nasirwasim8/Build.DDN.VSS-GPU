...
class ImageAnalyzer:
    ...
    def __init__(self):
        ...
        try:
            ...
            # BLIP for image captioning — on GPU (RTX 5090, sm_120)
    ...
    def generate_caption(self, image: Image.Image) -> str:
        """Generate image caption using BLIP (GPU with CPU fallback)."""
        ...
        try:
            ...
            # GPU kernel incompatibility — fall back to CPU silently
            try:
                self.caption_model = self.caption_model.to('cpu')
                self.device = 'cpu'  # Stay on CPU for rest of session
                inputs = self.caption_processor(image, return_tensors="pt")
                with torch.no_grad():
                    outputs = self.caption_model.generate(**inputs, max_new_tokens=50)
                return self.caption_processor.decode(outputs[0], skip_special_tokens=True)
            except Exception as e2:
                return f"Caption unavailable: {e2}"
    ...
    def generate_captions_batch(self, images: List[Image.Image]) -> List[str]:
        """
        Generate captions for a batch of images in ONE GPU call — much faster than sequential.

        Falls back to sequential single-image processing if batch fails,
        so captions are never replaced with error strings.
        """
        if not self.models_loaded or not images:
            return ["AI models not loaded"] * len(images)

        try:
            # Batch all images in a single forward pass
            ...
            with torch.no_grad():
                outputs = self.caption_model.generate(**inputs, max_new_tokens=50)
            return [
                self.caption_processor.decode(out, skip_special_tokens=True)
                for out in outputs
            ]
        except Exception as e:
            logger.warning(f"Batch captioning failed ({e}), falling back to sequential")
            # Fallback: process one at a time — still works, just slower
...
class DocumentAnalyzer:
    ...
    def __init__(self):
        ...
        if TRANSFORMERS_AVAILABLE and TORCH_AVAILABLE:
            # BART summarizer and BERT NER are disabled — they are not needed
            # for core VSS functionality (CLIP/BLIP handle video/image search).
            # Enabling them caused OOM crashes after image models loaded.
            # A simple extractive summarizer is used as fallback instead.
            print("Document analysis ready (using lightweight extractive mode)")
            self.summarizer = None
            self.ner_pipeline = None
    ...
    def summarize(self, text: str, max_length: int = 150) -> str:
        ...
        # If BART model available, use it
        if self.summarizer:
            try:
                if len(text) > 1024:
                    text = text[:1024]
